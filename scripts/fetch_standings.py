# -*- coding: utf-8 -*-
"""Fetch the live UCL 2026/27 league-phase table from football-data.org."""
...
VARIANTS = {
 ...
 "aek athens fc":"AEK Athens", "aek athens":"AEK Athens", "pae aek":"AEK Athens", "aek":"AEK Athens",
...
def main():
    ...
    try:
        ...
        print("ERROR: API returned HTTP %s — %s" % (e.code, e.read().decode("utf-8","ignore")[:300])); sys.exit(3)
    ...
    for row in best:
        ...
        rows.append({"rank": row.get("position"), "team": canon,
    ...
    out = {"season":"2026/27","competition":"UEFA Champions League","phase":"League phase",
        "matchday": md if md is not None else 0,
        "last_updated": datetime.datetime.now(datetime.timezone.utc).isoformat(timespec="seconds"),
        "source":"football-data.org","standings":rows}
    with open(OUT, "w", encoding="utf-8") as f:
        json.dump(out, f, ensure_ascii=False, indent=2)
    print("OK: wrote %d teams (matchday %s)" % (len(rows), out["matchday"]))
